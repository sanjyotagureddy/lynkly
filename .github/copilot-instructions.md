# Copilot Instructions for Lynkly Resolver

These instructions are mandatory for all changes in this repository.

## Repository Context

This solution is a .NET 10 URL shortener built around Minimal APIs, Clean Architecture, Vertical Slice Architecture (VSA), EF Core, and an event-driven analytics pipeline.

The intended structure uses these boundaries:
...
- `shared/Lynkly.Shared.Kernel.*`: reusable primitives that must stay NuGet-ready.
- `tests/*`: unit, integration, architecture, performance, and contract tests.
- `docs/*`, `docker/*`, `scripts/*`: documentation, local tooling, and environment assets.
...
4. Keep naming and folder placement consistent with the repo structure above.
...
3. Keep request validation near the feature, not in the transport layer only.
...
4. Prefer Conventional Commit style when practical (for example: `feat(links): add create-link validator`).
5. Stage only relevant files and review staged diffs before committing.
6. Do not commit generated artifacts, secrets, or local-only environment files.
7. Run `dotnet restore`, `dotnet build`, and `dotnet test` before commit when feasible.
8. If validation is skipped, explicitly state what was skipped and why.
9. Do not rewrite shared history or amend commits unless explicitly requested.

## Code Review / Output Expectations

When you make changes, always:

1. List the modified files and why they changed.
2. Call out assumptions explicitly.
3. Report validation commands you ran and their outcomes.
4. Say clearly if validation was skipped and why.
5. Avoid claiming completion if tests or builds are still unverified.
