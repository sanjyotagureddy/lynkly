...
public static class MathHelper
{
    ...
    public static decimal Clamp(decimal value, decimal minInclusive, decimal maxInclusive)
    {
        ...
        {
            throw SharedKernelException.InvalidArgument("Minimum value cannot be greater than maximum value.");
        }
    ...
    public static decimal Percentage(decimal part, decimal total, int decimals = 2)
    {
        if (total == 0)
        {
            throw SharedKernelException.InvalidArgument("Total cannot be zero when calculating percentage.");
        }
